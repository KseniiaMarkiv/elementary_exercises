...
def ascii_translator(number)
  # number = gets.chomp.to_i.chr  - it's for get number from user
  number.chr
end
...
def alphabetical_list games
  games.uniq.sort    # for performance is better when at first uniq method is used and then sort method
end
# you can check this go through 
...
def lucky_number number = 7
  number == 7 ? "Today's lucky number is 7" : "Today's lucky number is #{number}"
end
...
def ascii_code string
  string.length == 1 ? string.ord : 'Input Error'
end
