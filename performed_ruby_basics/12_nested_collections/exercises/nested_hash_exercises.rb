def find_language_information(languages, language_name)
  ...
  # }
  languages[language_name] if languages.has_key?(language_name)
...
find_language_information(hash, :noun)
...
def add_language(languages, language_name, language_info_value)
  ...
  # languages.store(language_name, language_info_value)  - second way
  languages
end
...
def find_language_facts(languages, language_name, fact_index = 0)
  ...
  # REVIEW one way by DIG method
  # languages.dig(language_name, :facts, fact_index)
  languages[language_name] && languages[language_name][:facts] ? 
...
p find_language_facts(hash_three, :ruby)  # "was made for programmer happiness"
# ! pointed on index
p find_language_facts(hash_three, :ruby, 1)  # "its code is beautiful"
