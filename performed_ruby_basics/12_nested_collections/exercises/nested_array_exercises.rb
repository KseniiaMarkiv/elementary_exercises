def blank_seating_chart(number_of_rows, seats_per_row)
  ...
  #  Array.new(number_of_rows) { Array.new(seats_per_row) }
    ...
    # Manually add rows to the chart
    number_of_rows.times do
      # Create a row with the specified number of seats, each initialized to nil
      # Add the row to the chart
      chart << Array.new(seats_per_row, nil)
    end
...
p add_seat_to_row(array, 0, 'Diane')
...
p add_another_row(array, ['Tim', 'Joe', nil])
...
array = [[nil, 'Bob', 'Joe'], [nil, nil, 'Bill']]
p delete_seat_from_row(array, 0, 1)
...
def count_empty_seats(chart)
  ...
  chart.sum { |row| row.count(nil) }
  # ?NOTE: `chart` should **not** be mutated
end
array = [[nil, 'Bob', 'Joe'], [nil, nil, 'Bill']]
# p array.sum    - ERROR Array can't be coerced into Integer (TypeError)
p count_empty_seats(array)
